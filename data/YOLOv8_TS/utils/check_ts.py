...
LEFT_ROI_START_X = 0
LEFT_ROI_START_Y = 0
LEFT_ROI_END_X = int(CAM_WIDTH * 1/5)
LEFT_ROI_END_Y = CAM_HEIGHT

RIGHT_ROI_START_X = int(CAM_WIDTH * 4/5)
RIGHT_ROI_START_Y = 0
RIGHT_ROI_END_X = CAM_WIDTH
RIGHT_ROI_END_Y = CAM_HEIGHT

TOP_ROI_START_X = 0
TOP_ROI_START_Y = 0
TOP_ROI_END_X = CAM_WIDTH
TOP_ROI_END_Y = int(CAM_HEIGHT * 1/2)


#class Check(left_roi:list, right_roi:list, top_roi:list, box_size_th:int, conf_th:float): 
class Check(): 
    def __init__(self):
        self.left_roi  = [LEFT_ROI_START_X, LEFT_ROI_START_Y, LEFT_ROI_END_X, LEFT_ROI_END_Y]
        self.right_roi = [RIGHT_ROI_START_X, RIGHT_ROI_START_Y, RIGHT_ROI_END_X, RIGHT_ROI_END_Y]
        self.top_roi = [TOP_ROI_START_X, TOP_ROI_START_Y, TOP_ROI_END_X, TOP_ROI_END_Y]
        self.box_size_threshold = 1800 #1800
        # self.box_size_threshold = 85 * 355 
        self.confidence_threshold = 0.72 # 0.7
    ...
    #0. Base Check. (all class apply)
        ...
        left_roi_ch = True if (self.left_roi[0] < self.center_x <= self.left_roi[2]) else False
        right_roi_ch = True if (self.right_roi[0] <= self.center_x < self.right_roi[2]) else False
    ...
    #2. Signal Case
        ...
        ch = ((left_roi_ch or right_roi_ch or top_roi_ch) and \
             # is_lr=="right" or 
             box_size_ch and \
             conf_ch)
    ...
    def draw_zone(self, img):
        ...
        #                     (LEFT_ROI_START_X, LEFT_ROI_START_Y),
        #                     (LEFT_ROI_END_X, LEFT_ROI_END_Y),
        #                     (255,0,20), # bgr
        #                     2)
        img = cv2.rectangle(img,
